...
class Message:
    ...
    def __init__(self, msg_type, content, sender_id):
        self.type = msg_type  # 'PROPOSE', 'VOTE', 'ECHO'
...
class Node(threading.Thread):
    ...
    def __init__(self, node_id, nodes):
        ...
        self.received_messages = set()  # For URB message deduplication
        self.is_active = True  # Indicates if the node is active (not crashed)
        self.crashed = False  # Indicates if the node has crashed before
        self.delay = random.uniform(0, 2)  # Random delay to simulate network latency
    ...
    def broadcast(self, message):
        """Sends a message to all other nodes with simulated delay and possible loss."""
        if not self.is_active:
            return
        for node in self.nodes:
            if node.node_id != self.node_id:
                # Simulate message delay and possible loss
                threading.Thread(target=self.delayed_send, args=(node, message)).start()
        print(f"Node {self.node_id}: Broadcasted {message.type} message.")

    def delayed_send(self, node, message):
        """Sends a message to a node after a delay."""
        # Simulate network delay
        delay = random.uniform(0, D * EPOCH_DURATION)
        time.sleep(delay)
        # Simulate message loss with a small probability
        if random.random() < 0.1:
            print(f"Node {self.node_id}: Message lost during transmission to Node {node.node_id}.")
            return  # Message is lost
        node.receive_message(Message('ECHO', message, self.node_id))
    ...
    def run(self):
        ...
        while True:
            if not self.is_active:
                ...
                time.sleep(random.uniform(1, 3))  # Down time before recovery
                self.is_active = True
            ...
            if self.node_id == leader_id and self.is_active:
                self.propose_block()
            ...
            if random.random() < 0.05 and self.is_active:
                print(f"Node {self.node_id} has crashed.")
                self.is_active = False
                self.crashed = True

            # Simulate nodes missing epochs
            if random.random() < 0.1:
                print(f"Node {self.node_id} is delayed and misses an epoch.")
                time.sleep(EPOCH_DURATION)  # Node misses one epoch
    ...
    def process_message(self, message):
        ...
        if message.type == 'ECHO':
            # URB Echo mechanism
            inner_message = message.content
            message_id = (inner_message.type, getattr(inner_message.content, 'hash', inner_message.content), inner_message.sender_id)
            if message_id not in self.received_messages:
                self.received_messages.add(message_id)
                self.broadcast(message)  # Echo the message
                self.handle_message(inner_message)
        else:
            self.handle_message(message)

    def handle_message(self, message):
        """Handles a non-ECHO message."""
...
def simulate_phase2():
    """Runs the simulation for Phase 2."""
    # Initialize nodes
