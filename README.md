...
2. Configure the network_info.json file with the desired values (num_node [int]; total_epochs[int]; delta[int]; start_time[hh:mm]; ports[List<int>]; confusion_start[String("HH:MM")], confusion_duration[int])
